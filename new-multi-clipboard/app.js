...
(function () {
  ...
  function load() {
    var base = { used: {}, last: {}, fav: [], theme: 'dark', tour: false, tagMode: 'or', sort: 'recommend', reqs: [] };
    try {
  ...
  function save() {
    try { localStorage.setItem(LS_KEY, JSON.stringify(store)); } catch (e) { /* 保存できなくても動く */ }
  }
  ...
  function buildPrompt() {
    ...
    L.push('');
    L.push('## 守ること');
    L.push('1. 変更してよいファイルは `' + REPO.file + '` だけです。同じフォルダの `index.html` / `styles.css` / `app.js` や、リポジトリ内の他のフォルダは触らないでください。');
